...
public class ByteBufferPool {
    ...
    private final ArrayList<ByteBuffer> fileQueue = new ArrayList<>();

    private boolean isWait = false;

    public ByteBufferPool(int memorySize, int fileSize, File file) throws IOException {
        if (memorySize > 0) initMemoryBuffer(memorySize);

        if (fileSize > 0) initFileBuffer(fileSize, file);
    }
    ...
    private void divideBuffer(ByteBuffer buf, int blockSize, ArrayList<ByteBuffer> list) {
    ...
    public void putBuffer(ByteBuffer buffer) {
        if (buffer.isDirect()) {
            if (buffer.capacity() == MEMORY_BLOCKSIZE) {
                putBuffer(buffer, memoryQueue);
            } else if (buffer.capacity() == FILE_BLOCKSIZE) {
